...
fun ProcurementModuleHost(
    ...
    SimpleCaptureScreen(
        ...
        extra = {
            ...
            androidx.compose.material3.Button(onClick={run { val quantity=qty.value.replace(',','.').toDoubleOrNull()?:error("Enter a quantity"); val major=amount.value.replace(',','.').toDoubleOrNull()?:error("Enter an amount"); ops.recordPurchase(RecordPurchase(purchaseId = UUID.randomUUID().toString(), supplierId = supplierId.value, itemId = itemId.value, quantityMilli = (quantity * 1000.0).toLong(), amountMinor = (major * 100.0).toLong(), occurredEpochDay = LocalDate.parse(day.value).toEpochDay()), newContext()) }},enabled=!busy.value&&supplierId.value.isNotBlank()&&itemId.value.isNotBlank()&&qty.value.isNotBlank()&&amount.value.isNotBlank()&&day.value.isNotBlank()){androidx.compose.material3.Text("Record purchase")}
        },
    )
}
