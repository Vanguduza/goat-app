...
Farm OS uses the owner-locked illustrated bespoke Animal Farm visual family. The current presentation authority is `docs/ux/animal-farm-visual-lock/`, starting at `docs/ux/animal-farm-visual-lock/START-HERE.md` and `docs/ux/ANIMAL_FARM_COLD_START_RESUME.md`. `docs/ux/FARM_OS_VISUAL_AUTHORITY.md` and earlier pastoral presentation material are provenance/inventory only wherever they conflict with the Layer 3 lock. The complete screen/feature/visual mapping remains `docs/ux/FARM_OS_QUANTUM_COMPLETE_SCREEN_FEATURE_VISUAL_MAPPING_REV2.md` for Screen IDs, feature scope, role/device law and evidence requirements, with conflicting presentation text superseded by Layer 3. Existing UI code is implementation evidence, not visual authority. Later field-first documents are superseded wherever they prohibit or contradict the canonical illustrated family; their field usability, accessibility, safety and evidence requirements remain binding. Every product screen must have a registered `FOS-*` Screen ID and visual lineage. Visual certification remains separate from feature/module/MVP green states.
